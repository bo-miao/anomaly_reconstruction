...
def main_worker(gpu, ngpus_per_node, args):
    ...
    log_path = os.path.join('runs', '_'.join([args.suffix, args.dataset_type,
...
def train(train_batch, model, optimizer, epoch, args):
    ...
    # else:
    #     prefetcher = prefetch.data_prefetcher_without_label(train_batch)
    ...
    while images is not None:
        ...
        with autocast():
            if 'Classifier' in args.arch:
                reconstructed_image, loss, logit = model.forward(input_image, gt=target_image, label=label, train=True)
                loss = loss['pixel_loss'].mean() + loss['discriminative_loss'].mean()
            else:
